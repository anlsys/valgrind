...
Still, measurements showed that memory use and execution time follow a similar shape with \code{s}, but with much larger overheads: for \code{-s=64}, before the application crashes in the first iteration, the execution time and memory use were respectively $79 s$ and $75GB$.
...
\begin{lstlisting}[style=text, captionpos=b, caption={An error report from Taskgrind}, label={lst:error-report-romp}]
	[...]
	data race found:
\end{lstlisting}

\begin{lstlisting}[style=text, captionpos=b, caption={An error report from ROMP}, label={lst:error-report-taskgrind}]
